...
def create_tag_annotations_links(conn, dataType, additions=[], removals=[]):
    """
    ...
    model_d = {
        ...
        "Well": (omero.model.WellI, omero.model.WellAnnotationLinkI)
    }

    new_links = []
    # Create a list of links to apply
    for addition in additions:
        link = model_d[dataType][1]()
        link.parent = model_d[dataType][0](addition[0], False)
    ...
    if len(removals) > 0:
        ...
        # on queries)
        all_obj_ids, all_tag_ids = list(zip(*removals))

        params = omero.sys.Parameters()
        params.theFilter = omero.sys.Filter()
        params.theFilter.ownerId = rlong(conn.getUserId())
        # This query gets all the relationships between these objects and these
        # tags, otherwise we'd have to get them individually.
        links = conn.getAnnotationLinks(
            dataType,
            parent_ids=list(all_obj_ids),
            ann_ids=list(all_tag_ids),
            params=params,
        )

        # The above returns more object->tag links that were specified for
